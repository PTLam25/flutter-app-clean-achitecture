...
# Dependencies injections get_it
get_it позволяет получить доступ к объектам и любой части дерево виджетов, тем самым нам не надо в виджетах сохранять и перебрасывать доступы к объектам: блоку, usecases, utils классам и т.д.
 Хорошая статья: https://www.burkharts.net/apps/blog/one-to-find-them-all-how-to-use-service-locators-with-flutter/

 Суть get_it такая: get_it сам по себе является сиглтон Map (глобальным переменным), который хранится в памяти при работы приложения.  
 При инициализации приложения мы в это словарь региструем наши классы(типы), к которым мы хотим получить доступ через get_it.

 В виджете при обращения к get_it, мы просим дать объект типа такого и он дам его достает.


## Способы регистрации типов
### factory
Если вы зарегистрируете свой тип таким через **factory**, каждый вызов sl.get () будет создавать новый экземпляр.
Это часто используется для bloc, так как когда виджет удаляется с памяти, мы хотим, чтобы его bloc тоже полностью удалился.  
Если же регистрировать через **singleton**, то один и тот же bloc будет использоваться для разных объектов виджетов. И при удаления первого, то bloc будет иметь последний его state, при создания другого виджета, вместо initial state будет тот.
Поэтому bloc должен быть зарегистрированным как **factory**.

### singleton
Singleton создает только один объект, который будет использоваться во всей приложенни.

#### lazySingleton
Поскольку создание экземпляра при регистрации может занять много времени при запуске приложения, вы можете перенести создание на то время, когда объект запрашивается в первый раз, с помощью lazySingleton.